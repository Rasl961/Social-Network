...
//    This friend add me
 ...
 app.post('/choosefriendtomessage', function(req,res){
    ...
    sess.friendsroom = req.body.friendid;
    sess.friendslogin = req.body.friendlogin;

    console.log(sess.friendsroom);
    res.sendStatus(200);
 });
...
io.on('connection', function(socket) {
 ...
 console.log(myroom);
    ...
    if(typeof sess == 'object') {
        socket.join(sess.friendsroom);
    socket.on('message', function(data){
        socket.broadcast.to(sess.friendsroom).emit('message', {  
         
                nickname: login,
                message: data.message,
                rooms: sess.friendslogin
             })
    });
    }});
